import random
from backend.src.basic_tools.data_structure.mapper import UnidirectionalOneToOneMapping
...
class FileOperators:
    ...
    @staticmethod
    def get_data_from_cache_file_if_there_is_any_else_create_and_save_cache_file(cache_dir, cache_file_type,
                                                                                 function_for_creation_if_needed,
                                                                                 params_for_creation_if_needed):
        """
        Args:
            cache_dir: a str
            cache_file_type: a str
            function_for_creation_if_needed: a function
            params_for_creation_if_needed: an arg dict

        Returns:
            a data that follows the cache_file_type
        """
        assert cache_file_type in ['json', 'txt']
        from os import path
        from backend.src.basic_tools.data_loader import JSONLoader, JSONSaver, TEXTLoader, TEXTSaver
        if path.isfile(cache_dir):
            print(f"[Loading] {cache_dir}")
            if cache_file_type == 'json':
                to_return = JSONLoader(dir_=cache_dir)()
            elif cache_file_type == 'txt':
                to_return = TEXTLoader(dir_=cache_dir)()
            else:
                raise TypeError
        else:
            to_return = function_for_creation_if_needed(**params_for_creation_if_needed)
            print(f"[Saving] {cache_dir}")
            if cache_file_type == 'json':
                JSONSaver(dir_=cache_dir, data_=to_return)()
            elif cache_file_type == 'txt':
                TEXTSaver(dir_=cache_dir, data_=to_return)()
            else:
                raise TypeError
        return to_return
...
class DocumentOperators:
    ...
    def get_sample_document_for_safe_keywords(self):
        """
        Returns:
            a list of str
        """
        ...
        for keyword in safe_keywords:
            try:
                documents += self.searcher(query=keyword)
            except RuntimeError:
                pass
        documents = [x['doc_text'] for x in documents]
        return documents

    @staticmethod
    def load_searcher():
        from backend.src.searcher import SearcherLoader
        searcher = SearcherLoader()()
        return searcher


class OccurrenceFrequencyOperators:
    def __init__(self):
        pass

    @staticmethod
    def normalize(freq_dict):
        """
        Args:
            freq_dict: a dict that has the key of a str and the value of an int

        Returns:
            a dict that has the key of a str and the value of an int
        """
        sum_ = int(1)
        num_key = int(1)
        for key_, val_ in freq_dict.items():
            num_key += 1
            sum_ += val_
        normalized_freq_dict = dict()
        for key_, val_ in freq_dict.items():
            rev_val_ = int(float(num_key*val_)/float(sum_))
            if rev_val_ == 0:
                continue
            else:
                pass
            normalized_freq_dict[key_] = int(rev_val_)
        return normalized_freq_dict

    def get_the_doc_with_the_highest_occurrence_frequency_overall_among_given_documents(self, documents, keyword_list):
        """
        Args:
            documents: a list of str
            keyword_list: a list of str
        ...
        for document in documents:
            document2freq[document] = \
                self.get_accumulated_occurrence_frequency_of_keyword_list_within_a_document(
                    keyword_list=keyword_list, document=document
                )
        to_return = max(documents, key=lambda x: document2freq[x])
        return to_return

    @staticmethod
    def get_accumulated_occurrence_frequency_of_keyword_list_within_a_document(keyword_list, document):
        """
        Args:
            keyword_list: a list of str
            document: a str
        ...
        for keyword in keyword_list:
            if keyword in document:
                to_return += document.count(keyword)
            else:
                continue
        return to_return

    @staticmethod
    def get_if_given_keywords_are_all_within_a_document(keyword_list, document):
        """
        For example, if the keyword list is ['lucky', 'strike', 'candy', 'alpha']

        Then, if the document is "the lucky candy means alpha of strike", then this method will return True.
        if the document is "the merciful candy means beta of alpha strike", then this method will return False,
        as the 'lucky' is missing. It should be noted that word boundaries are not considered.
        ...
        Returns:
            a bool, True if all the keywords are within the document, False else.
        """
        to_return = True
        for keyword in keyword_list:
            if keyword in document:
    ...
    def calculate_n_gram_occurrence_frequency_from_text_bundle(self, text_bundle, window_size,
                                                               whether_save_to_json_file=False, save_file_dir=None):
        """
        Args:
            text_bundle: a list of str
            window_size: an int
            whether_save_to_json_file: a bool
            save_file_dir: a str or None,
                           if it is remained as None with the 'whether_save_to_json_file' flag of True,
                           it may raise an assertion error.

        Returns: a dict where the key is the n_gram and the value is the occurrence frequency.
        """
        if whether_save_to_json_file:
            assert save_file_dir, (
                f"the save_file_dir should be set but it is now {save_file_dir}"
            )
        overall_frequency_dict = dict()
        from tqdm import tqdm
        idx = 0
        print(f"[Iteration] updating occurrence frequency dictionary Iterating Over the articles")
        for text in tqdm(text_bundle):
            n_gram_occurrence_frequency = self.get_occurrence_frequency_of_the_n_grams(
                window_size=window_size, text=text
            )
            if idx == 0:
                overall_frequency_dict = n_gram_occurrence_frequency
                idx += 1
            else:
                self.merge_occurrence_frequency_dict(
                    dict_to_add=n_gram_occurrence_frequency, dict_to_update=overall_frequency_dict
                )
        overall_frequency_dict = [{"n_gram": x, "occurrence_frequency": overall_frequency_dict[x]}
                                  for x in sorted(list(overall_frequency_dict.keys()),
                                                  key=lambda y: overall_frequency_dict[y],
                                                  reverse=True)]
        if whether_save_to_json_file:
            from ..basic_tools.data_loader import JSONSaver
            JSONSaver(dir_=save_file_dir, data_=overall_frequency_dict)()
        else:
            pass
        return overall_frequency_dict

    @staticmethod
    def merge_occurrence_frequency_dict(dict_to_add, dict_to_update):
        """
        Args:
            dict_to_add: a dict, which is an occurrence frequency dict, key = string and the value is integer.
            dict_to_update: a dict, having the same format as the `dict_to_add`

        Returns:
            a dict, which is the updated dict
        """
        for key_1, val_1 in dict_to_add.items():
            if key_1 in dict_to_update:
                dict_to_update[key_1] = val_1 + dict_to_update[key_1]
            else:
                dict_to_update[key_1] = val_1
        return dict_to_update

    def get_occurrence_frequency_of_the_n_grams_with_multiple_window_sizes(self, list_of_window_size, text,
                                                                           whether_allowing_only_given_n_grams=False,
                                                                           allowed_n_grams=None,
                                                                           must_include_the_keywords=None):
        """
        Args:
            list_of_window_size: a list of int
            text: a str
            whether_allowing_only_given_n_grams: a bool, if true, only the str in `allowed_n_grams` can be
                                                 included in the extracted occurrence frequency dict.
                                                 in other words, if the n_gram is not in the allowed ones,
                                                 it will not be counted by this method
            allowed_n_grams: a list of str
            must_include_the_keywords: a list of keywords or None

        Returns:
            a dict where the key is the n-gram and the value is the occurrence frequency
        """
        freq_dict_to_return = dict()
        for window_size in list_of_window_size:
            freq_dict_to_add = self.get_occurrence_frequency_of_the_n_grams(
                window_size=window_size, text=text,
                whether_allowing_only_given_tokens=whether_allowing_only_given_n_grams,
                allowed_tokens=allowed_n_grams, must_include_the_keywords=must_include_the_keywords
            )
            freq_dict_to_return = self.merge_occurrence_frequency_dict(
                dict_to_add=freq_dict_to_add, dict_to_update=freq_dict_to_return
            )
        return freq_dict_to_return

    @staticmethod
    def get_occurrence_frequency_of_the_n_grams(window_size, text, whether_allowing_only_given_tokens=False,
                                                allowed_tokens=None, must_include_the_keywords=None):
        """
        Args:
            window_size: an int
            text: a str
            whether_allowing_only_given_tokens: a bool, if true, only the str in `allowed_tokens` can be
                                                 included in the extracted occurrence frequency dict.
                                                 in other words, if the n_gram contains a token
                                                 which is not within the allowed ones,
                                                 it will not be counted by this method.
            allowed_tokens: a list of str
            must_include_the_keywords: a list of keywords or None

        Returns:
            a dict where the key is the n-gram and the value is the occurrence frequency
        """
        def whether_one_of_the_keywords_is_in_the_given_list_of_tokens(list_of_keywords, list_of_tokens):
            """
            Args:
                list_of_keywords: a list of str
                list_of_tokens: a list of str

            Returns:
                a bool
            """
            for keyword in list_of_keywords:
                if keyword in list_of_tokens:
                    return True
            return False
        from backend.src.basic_tools.data_structure.dict_of_list import DictOfList
        list_op = ListOperators()
        plain_text_op = PlainTextOperators()
        token_list = plain_text_op.naive_split(text)
        n_gram_list = list_op.get_n_gram(list_=token_list, window_size=window_size)
        if must_include_the_keywords:
            n_gram_list = [list_of_tokens for list_of_tokens in n_gram_list
                           if whether_one_of_the_keywords_is_in_the_given_list_of_tokens(
                              must_include_the_keywords, list_of_tokens
                           )]
        dict_to_convert, occurrence_frequency_dict = dict(), DictOfList({})
        for n_gram in n_gram_list:
            if whether_allowing_only_given_tokens:
                if len([x for x in n_gram if x in allowed_tokens]) == len(n_gram):
                    occurrence_frequency_dict.add_new_key_value("|".join(n_gram), 1)
            else:
                occurrence_frequency_dict.add_new_key_value("|".join(n_gram), 1)
        for key_, val_list in occurrence_frequency_dict.get_key_val_iterator():
            val_total = sum(val_list)
            if val_total > 1:
                pass
            else:
                continue
            dict_to_convert[key_] = val_total
...
class RandomOperators:
    ...
    @staticmethod
    def randomly_select_items_from_list(list_, selection_size):
        """
        Args:
            list_: list of items, to select from
            selection_size: the size of the list to be returned

        Returns:
            a list
        """
        random.shuffle(list_)
        list_ = list_[:selection_size]
        return list_
